%% 3rd example from Sullivan's book
clear; clc; close all;
...
for n = 1:NSTEPS
    ...
    % Gaussian pulse at the low end
    ex(5) = ex(5) + exp(-0.5 * ((t0 - n) / spread)^2); % Soft
    ...
    set(hEx, 'YData', ex);
    set(hHy, 'YData', hy);
    sgtitle(sprintf('1D FDTD, Gaussian pulse, striking dielectric, ABC, step %.0f', n));
    drawnow;
end
